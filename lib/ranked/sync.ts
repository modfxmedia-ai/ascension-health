import { hasRankedApiKey, listRankedProjects, resolveThisSiteProjectId } from "./client"
...
export async function syncAllRankedSites() {
  ...
  const thisSiteId = await resolveThisSiteProjectId()
  const targets = getRankedSiteTargets(thisSiteId ?? undefined)
